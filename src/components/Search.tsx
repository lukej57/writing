  ...
  return (
    ...
    >
      <div
        id={`${id}-title`}
        aria-hidden="true"
        className="text-sm text-slate-700 group-aria-selected:text-sky-600"
      >
      ...
      {hierarchy.length > 0 && (
        <div
          id={`${id}-hierarchy`}
          aria-hidden="true"
          className="mt-0.5 truncate text-xs whitespace-nowrap text-slate-500"
        >
  ...
  if (collection.items.length === 0) {
    return (
      <p className="px-4 py-8 text-center text-sm text-slate-700">
        ...
        <span className="wrap-break-word text-slate-900">
          {query}
        </span>
        &rdquo;
      </p>
...
>(function SearchInput({ autocomplete, autocompleteState, onClose }, inputRef) {
  ...
  return (
    <div className="group relative flex h-12">
      <SearchIcon className="pointer-events-none absolute top-0 left-4 h-full w-5 fill-slate-400" />
      <input
        ref={inputRef}
        data-autofocus
        className={clsx(
...
export function Search() {
  ...
  return (
    <>
      <button
        type="button"
        className="group flex h-6 w-6 items-center justify-center sm:justify-start md:h-auto md:w-80 md:flex-none md:rounded-lg md:py-2.5 md:pr-3.5 md:pl-4 md:text-sm md:ring-1 md:ring-slate-200 md:hover:ring-slate-300 lg:w-96"
        {...buttonProps}
      >
