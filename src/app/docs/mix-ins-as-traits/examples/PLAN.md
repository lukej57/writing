...
| Controllers (same `authorize!`) | `include AuthorizesRoster` **or** `RosterAuth.call(self)` **or** a lambda the method fires | Leave `authorize!` on each host. `Ability` is the policy. If it grows: `RosterAccess.new(user, roster)` — never the controller. | Authorization. Rung 1 only if the function is *pure*. Passing `self` is the tell. Not an endpoint. |
| Controllers (real trait) | `include Exportable` that also holds finders, `authorize!`, and `as_json` | `include CsvExport` — provided `#export`; host writes `export_scope`. Work of building the CSV may still be a collaborator. | Orthogonal endpoint. The include list reads as actions, not helpers. |
...
| Long `User` split into `UserAuth` / `UserBilling` | `include` only on `User` (`concerning` or `app/models/concerns/`) | Leave methods on `User`, or extract collaborators other types actually use | God class carved into single-use concerns — still one object |
