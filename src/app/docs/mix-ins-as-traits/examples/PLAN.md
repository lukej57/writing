...
| Controllers (same `authorize!`) | `include AuthorizesRoster` **or** `RosterAuth.call(self)` **or** a lambda the method fires | Leave `authorize!` on each host. `Ability` is the policy. If it grows: `RosterAccess.new(user, roster)` — never the controller. | Authorization. Rung 1 only if the function is *pure*. Passing `self` is the tell. |
