...
- `PREVIEW.md` — chapter preview: the argument, three parts, locked decisions, what's next. Start here after a break.
...
6. Rails leftover: the framework occupies SI on MVC, so mixin-for-everything is the default if logic stays there; pull work into objects you own. Point at *The One Job of a Concern*.
